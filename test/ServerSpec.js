...
describe('', function () {
  ...
  describe('Link creation:', function () {
    ...
    describe('Creating new links:', function (done) {
      ...
      it('Increments visits when a user uses a code', function (done) {
        var options = {
          'method': 'POST',
          'followAllRedirects': true,
          'uri': 'http://127.0.0.1:4568/signup',
        ...
        requestWithSession(options, function (error, res, body) {
          if (error) { return done(error); }
          db.query('insert into links (id, url, baseUrl, code, title, visits) values (1, "http://www.google.com/", "http://127.0.0.1:4568/", "12345", "Google", 0);', function (err) {
            if (err) { return done(err); }
            var options = {
              'method': 'GET',
              'uri': 'http://127.0.0.1:4568/12345'
            };
            requestWithSession(options, function (error, res, body) {
              if (error) { return done(error); }
              db.query('SELECT * FROM links where code = "12345";', (err, nextLinks) => {
                expect(nextLinks[0].visits).to.equal(1);
                done();
              });
            });
          });
      ...
      it('Code should not increment if using an invalid code', function (done) {
        var options = {
          'method': 'POST',
          'followAllRedirects': true,
          'uri': 'http://127.0.0.1:4568/signup',
        ...
        requestWithSession(options, function (error, res, body) {
          ...
          db.query('insert into links (id, url, baseUrl, code, title, visits) values (1, "http://www.google.com/", "http://127.0.0.1:4568/", "12345", "Google", 0);', function (err) {
            ...
            requestWithSession(options, function (error, res, body) {
              if (error) { return done(error); }
              db.query('SELECT * FROM links where code = "54321";', (err, nextLinks) => {
                expect(nextLinks.length).to.equal(0);
                done();
              });
            });
          });
      ...
      it('Creates clicks table entry for a valid link', function (done) {
        db.query('SELECT * FROM links;', (err, clicks1) => {
          //create a new link
          var options = {
            'method': 'POST',
            'followAllRedirects': true,
          ...
          requestWithSession(options, function (error, res, body) {
            ...
            requestWithSession(options, function(error, res, body) {
              //query db for clicks
              db.query('SELECT * FROM links', (error, response) => {
                console.log('Clicks got from db: ', response);
                // var options = {
                //   'uri': 'http://127.0.0.1/' + response[0].code
                // };
                request('http://127.0.0.1:4568/' + response[0].code, function(error, res, body) {
                  db.query('SELECT * FROM clicks;', (err, clicks2) => {
                    console.log('clicks1.length: ', clicks1.length);
                    console.log('clicks2.length: ', clicks2.length);
                    expect(clicks2.length).to.equal(clicks1.length + 1);
                    done();
                  });
